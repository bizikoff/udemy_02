...
window.addEventListener('DOMContentLoaded', () => {
  ...
  class MenuCard {
    constructor(src, alt, title, descr, price, parent, ...classes) {
      ...
      this.descr = descr;
      this.price = price;
      this.classes = classes;                           // rest оператор возвращает массив
    ...
    render() {                                        // метод рендер для создания карточки
      ...
      if (this.classes.length === 0) {                // проверяем на наличие данных в rest операторе (массиве)
        this.classes = 'menu__item';                  
        element.classList.add(this.classes);          // задаем значение класса по дефолту 
      } else {            // если значение прописанно в рест операторе, перебираем массив и пушим в классы элемента
